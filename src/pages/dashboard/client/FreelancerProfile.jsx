        ...
        <div className="space-y-3">
          {items.map((job, i) => (
            <div key={i} className="p-3 border border-gray-100 rounded-lg text-sm">
              ...
              <p className="text-gray-500 text-xs mt-0.5">{job.date || "Date not provided"}</p>
            </div>
          ))}
        </div>
      )}
...
const FreelancerProfile = () => {
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 gap-2">
        <p className="text-gray-700 font-medium text-sm">Could not load profile</p>
  ...
  const workExperience   = Array.isArray(profile.experiences)   ? profile.experiences   : [];
  const certifications   = Array.isArray(profile.certifications)   ? profile.certifications   : [];
  const otherExperiences = Array.isArray(profile.otherExperiences) ? profile.otherExperiences : [];
  const completed        = Array.isArray(profile.workHistory?.completed)  ? profile.workHistory.completed  : [];
  const inProgress       = Array.isArray(profile.workHistory?.inProgress) ? profile.workHistory.inProgress : [];
      ...
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 p-6 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <div className="flex gap-6">
              ...
              <div className="w-28 h-28 rounded-full overflow-hidden bg-gray-200 border-2 border-gray-100">
                {profile.photo ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-3xl font-bold text-teal-600 bg-teal-50">
                    {profile.firstName + " " + profile.lastName ? profile.firstName.slice(0, 2).toUpperCase() : "?"}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex-1 min-w-0">
              {profile.title ? (
                <p className="text-lg font-bold text-gray-900 leading-snug">{profile.title}</p>
              ) : (
              ...
              {profile.bio ? (
                ...
                <p className="text-sm text-gray-400 mt-2 italic">No bio added.</p>
              )}
            </div>
          </div>
            ...
            <div>
              <div className="flex items-center gap-1.5">
                <span className="text-base font-bold text-gray-900">
                  {profile.firstName + " " + profile.lastName || "Name not provided"}
                </span>
                {profile.verified && (
              ...
              {profile.address?.city || profile.address?.state ? (
                <div className="flex items-center gap-1 mt-0.5">
                  ...
                  <p className="text-xs text-gray-500">{profile.address?.city}, {profile.address?.state}</p>
                </div>
              ) : (
                <p className="text-xs text-gray-400 mt-0.5 italic">Location not provided</p>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="space-y-2">
                {languages.map((l, i) => (
                  <div key={i} className="text-sm flex justify-between">
                    <span className="font-semibold text-gray-800">{l.language || "........."}</span>
            ...
            <div className="mt-6">
              <h2 className="text-base font-bold text-gray-900 mb-3">Education</h2>
              {education.length === 0 ? (
                ...
                <div className="space-y-3">
                  {education.map((e, i) => (
                    <div key={i} className="text-sm border-b border-gray-50 pb-3 last:border-0 last:pb-0">
                      <p className="font-bold text-gray-800">{e.institution || "Institution not provided"}</p>
                      <p className="text-gray-500 text-xs">Degree: {e.degree || "Degree not provided"}</p>
                      {e.fieldOfStudy && <p className="text-gray-400 text-xs mt-0.5">Field of Study: {e.fieldOfStudy} </p>}
                      {e.passingYear && <p className="text-gray-400 text-xs mt-0.5">Passing Year: {e.passingYear} </p>}
        ...
        {/* ── WORK EXPERIENCE ───────────────────────────────────────────── */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <h2 className="text-base font-bold text-gray-900 mb-4">Work experience</h2>
            ...
            <div className="space-y-5">
              {workExperience.map((exp, i) => (
                <div key={i} className="pb-5 border-b border-gray-50 last:border-0 last:pb-0">
                  ...
                  {exp.startDate && (
                    <p className="text-xs text-gray-500 mt-0.5">{formatTimeAgo(exp.startDate)} - {formatTimeAgo(exp.endDate) || "Present"}</p>
                  )}
                  {exp.description && (
            ...
            <div className="space-y-4">
              {certifications.map((cert, i) => (
                <div key={i} className="pb-4 border-b border-gray-50 last:border-0 last:pb-0">
                  ...
                  {cert.issuer && (
                    <p className="text-gray-500 text-sm mt-0.5">{cert.issuer}</p>
                  )}
                  {cert.period && (
                    <p className="text-gray-400 text-xs mt-0.5">{cert.period}</p>
                  )}
                </div>
              ))}
            </div>
        ...
        {/* ── OTHER EXPERIENCES ─────────────────────────────────────────── */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <h2 className="text-base font-bold text-gray-900 mb-4">Other experiences</h2>
            ...
            <div className="space-y-3">
              {otherExperiences.map((exp, i) => (
                <div key={i} className="text-sm">
                  <p className="font-bold text-gray-800">{exp.title || "Title not provided"}</p>
                  {exp.description && (
