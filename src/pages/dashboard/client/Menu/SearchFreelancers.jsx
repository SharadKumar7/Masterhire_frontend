...
import { Check, ChevronDown, Heart, Search, X, SlidersHorizontal, Users, RotateCcw } from "lucide-react";
import { useNavigate } from "react-router-dom";
const apiUrl = import.meta.env.VITE_API_URL;

const CATEGORY_MAP = {
  "Accounting & Consulting": [
    "Personal & Professional Coaching", "Accounting & Book-keeping",
    "Financial Planning", "Recruiting & Human Resources",
    "Management Consulting", "Other - Accounting & Consulting",
  ],
  "Admin Support": [
    "Data Entry & Transcription Services", "Virtual Assistance",
    "Project Management", "Market Research & Product Reviews",
  ],
  "Data Science & Analytics": [
    "Data Analysis & Reporting", "Data Extraction/ETL",
    "Data Mining & Management", "AI & Machine Learning",
  ],
  "Design & Creativity": [
    "Art & Illustration", "Branding & Logo Design",
    "NFT, AR/VR & Game Art", "Graphic, Editorial & Presentation Design",
    "Photography", "Product Design", "Video & Animation",
  ],
  "IT & Networking": [
  ...
  "Sales Marketing": [
    "Digital Marketing", "Lead Generation & Telemarketing",
    "Marketing, PR & Brand Strategy",
  ],
  "Web, Mobile & Software Dev": [
    "Blockchain, NFT & Cryptocurrency", "AI Apps & Integration",
    "Desktop Application Development", "Ecommerce Development",
    "Game Design & Development", "Mobile Development",
    "Other-Software Development", "QA Testing",
    "Web & Mobile Design", "Web Development",
  ],
};
...
const HeartButton = ({ profileId, initialSaved = false }) => {
  ...
  return (
    ...
    bg-teal-700 text-white text-[10px] font-medium px-2 py-1 rounded-md
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      Object.entries(filters).forEach(([key, value]) => {
        ...
        if (!value || value === "All" || value === "Any job success" || value === "Any level") return;
        if (key === "rating") { const num = parseInt(value); if (!isNaN(num)) params.append("rating", num); return; }
        if (key === "jobSuccess") { const num = parseInt(value); if (!isNaN(num)) params.append("jobSuccess", num); return; }
        if (key === "english") { params.append("englishLevel", value); return; }
        if (key === "skill") { params.append("skill", value); return; }
        if (key === "available") { params.append("available", "true"); return; }
        if (key === "consultation") { params.append("consultation", "true"); return; }
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-teal-900 tracking-tight">Find Talent</h1>
            <p className="text-slate-500 mt-1 text-sm">Discover skilled freelancers for your next project.</p>
          </div>
          {!isFiltersDefault && (
            <button
            ...
            <div className="px-6 pb-6 border-t border-slate-100">
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 mt-5">
                ...
                <div className="space-y-3">
                  <FilterGroup label="Category" variant="none"
                    options={["All", ...Object.keys(CATEGORY_MAP)]}
                    activeValue={filters.category} defaultValue="All"
                    onSelect={(cat) => setFilters((prev) => ({ ...prev, category: cat, subcategory: "All" }))}
                    {...filterGroupProps}
                  />
                  {filters.category !== "All" && (
                    <FilterGroup label="Sub-Category" variant="none"
                      options={["All", ...CATEGORY_MAP[filters.category]]}
                      activeValue={filters.subcategory} defaultValue="All"
                      onSelect={(val) => setFilters((prev) => ({ ...prev, subcategory: val }))}
                      {...filterGroupProps}
                    />
                  )}
                </div>
                ...
                {/* Skills */}
                <div>
                  <FilterGroup label="Skills" variant="square"
                    options={["All", "React", "Node.js", "Python", "Figma", "Marketing"]}
                    activeValue={filters.skill} defaultValue="All"
                    onSelect={(val) => setFilters((prev) => ({ ...prev, skill: val }))}
                    {...filterGroupProps}
                  />
                </div>
                ...
                {/* Rating + Job Success */}
                <div className="space-y-3">
                  <FilterGroup label="Rating" variant="circle"
                    options={["All", "5", "4", "3"]}
                    activeValue={filters.rating} defaultValue="All"
                    onSelect={(val) => setFilters((prev) => ({ ...prev, rating: val }))}
                    {...filterGroupProps}
                  />
                  <FilterGroup label="Job Success" variant="circle"
                    options={["Any job success", "70% & up", "80% & up", "90% & up"]}
                    activeValue={filters.success} defaultValue="Any job success"
                    onSelect={(val) => setFilters((prev) => ({ ...prev, success: val }))}
                    {...filterGroupProps}
                  />
                </div>
                ...
                {/* Language */}
                <div className="space-y-3">
                  <FilterGroup label="English" variant="square"
                    options={["Any level", "Basic", "Conversational", "Fluent", "Native or bilingual"]}
                    activeValue={filters.english} defaultValue="Any level"
                    onSelect={(val) => setFilters((prev) => ({ ...prev, english: val }))}
                    {...filterGroupProps}
                  />
                  <FilterGroup label="Other Language" variant="square"
                    options={["All", "Hindi", "Bengali", "Tamil", "Gujarati", "Marathi", "Punjabi", "Kannada"]}
                    activeValue={filters.language} defaultValue="All"
                    onSelect={(val) => setFilters((prev) => ({ ...prev, language: val }))}
                    {...filterGroupProps}
                  />
                </div>
                ...
                <div className="flex flex-col gap-4 pt-1">
                  <CustomCheckbox label="Available Now" checked={filters.available}
                    onChange={(val) => setFilters((prev) => ({ ...prev, available: val }))} />
                  <CustomCheckbox label="Offer Consultations" checked={filters.consultation}
        ...
        {!loading && (
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-sm text-slate-500">
              ...
              <span>
                <span className="font-bold text-slate-800">{items.length}</span> freelancer{items.length !== 1 ? "s" : ""} found
              </span>
            </div>
            {items.length > 0 && (
                ...
                <div className="flex flex-col md:flex-row gap-5 pr-10">
                  ...
                  <div className="shrink-0">
                    {item.image ? (
                      <img src={item.image} alt={item.fullName}
                        className="w-18 h-18 rounded-2xl object-cover border-2 border-slate-100 w-[72px] h-[72px]" />
                    ) : (
                  ...
                  <div className="flex-grow min-w-0">
                    ...
                    <div className="mb-3">
                      ...
                      <p className="text-xs text-slate-400 font-medium mt-0.5">{item.location || "Location Private"}</p>
                    </div>
