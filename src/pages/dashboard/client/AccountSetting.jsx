...
const AddFundsModal = ({ onClose, onSuccess }) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const data = await paymentApi("/topup/create-order", "POST", { amount: num });
          ...
          {!submitted ? (
            <>
              ...
              <button
                ...
                {submitting ? "Opening Payment..." : `Add ₹${amount ? Number(amount).toLocaleString("en-IN") : "0"} to Wallet`}
              </button>
            ...
            <div className="py-6 text-center space-y-2">
              ...
              <p className="text-xs text-slate-500">₹{Number(amount).toLocaleString("en-IN")} has been added to your wallet.</p>
            </div>
          )}
        </div>
      </div>
...
const ChangePasswordModal = ({ onClose, onSave, saving }) => {
  ...
  const handleSubmit = () => { if (validate()) onSave(form.currentPassword, form.newPassword); };
  ...
  const fetchWalletData = async () => {
    try {
      ...
      const history = (txRes.transactions || [])
        .filter(t => ["Escrow Deposit", "Wallet Top-up", "Milestone Release"].includes(t.type))
        .map(t => ({
          _id:         t._id,
          amount:      t.amount,
          date:        t.dateValue || t.createdAt,
          status:      t.status === "Held" || t.status === "Completed" || t.status === "Released" ? "Success" : "Pending",
        ...
        return (
          <div className={`space-y-6 ${blurClass}`}>
            ...
            <div className="relative flex items-center gap-6">
              ...
              {editingField === "profilePhoto" && (
                <div className="absolute left-0 top-full mt-2 w-80 bg-white p-3 border rounded-lg shadow-lg z-10">
                  ...
                  <input type="text" className="w-full border p-2 rounded text-sm outline-none focus:border-teal-500"
                    value={draft.profilePhoto || ""}
                    onChange={(e) => setDraft({ ...draft, profilePhoto: e.target.value })}
                    placeholder="https://..." autoFocus />
                  <div className="flex gap-2 mt-2">
                    <button onClick={() => setEditingField(null)} className="flex-1 py-1.5 bg-teal-600 text-white rounded text-xs font-bold">Done</button>
                    <button onClick={() => { setDraft({ ...draft, profilePhoto: "" }); setEditingField(null); }} className="flex-1 py-1.5 border rounded text-xs text-gray-500">Clear</button>
                  </div>
                </div>
              )}
                ...
                {["fullName", "email"].map((field) => (
                  <div key={field} className="flex justify-between items-center p-4 border rounded-lg">
                    <div className="flex-grow">
                      <p className="text-sm text-gray-500">{field === "fullName" ? "Name" : "Email"}</p>
                      {editingField === field ? (
                        <input className="font-medium border-b border-teal-500 outline-none w-full mt-1"
                          value={draft[field] || ""} onChange={(e) => setDraft({ ...draft, [field]: e.target.value })} autoFocus />
                      ) : (
            ...
            {["companyName", "phone", "location"].map((field) => (
              <div key={field} className="flex justify-between items-center p-4 border rounded-lg">
                <div className="flex-grow">
                  <p className="text-sm text-gray-500 capitalize">{field === "companyName" ? "Company Name" : field === "phone" ? "Phone" : "Location"}</p>
                  {editingField === field ? (
                    <input className="font-medium border-b border-teal-500 outline-none w-full mt-1"
            ...
            <div className="flex justify-between items-start p-4 border rounded-lg">
              ...
              {editingField !== "bio" && <Edit2 size={18} className="text-gray-400 cursor-pointer mt-1" onClick={() => setEditingField("bio")} />}
            </div>
            ...
            <div className="pt-6 border-t">
              ...
              <p className="text-sm text-gray-500 mb-4">Closing your account is permanent. Please ensure all payments and activities are completed first.</p>
      ...
      case "Hiring Preferences":
        return (
          <div className={`space-y-6 ${blurClass}`}>
            ...
            {[
              { field: "freelancerLevel", label: "Preferred Freelancer Level", type: "select", options: ["", "Beginner", "Intermediate", "Expert"] },
              { field: "budgetRange",     label: "Preferred Budget Range",      type: "input",  placeholder: "e.g. ₹5,000 - ₹20,000" },
              { field: "communicationPreference", label: "Communication Preference", type: "select", options: ["", "Chat", "Video Call", "Email"] },
              { field: "jobVisibility",   label: "Job Visibility",              type: "select", options: ["", "Public", "Private", "Invite Only"] },
          ...
          <div className={`space-y-8 ${blurClass}`}>
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-between items-center py-2">
                ...
                <button onClick={() => setShowPasswordModal(true)} className="text-teal-600 font-bold hover:underline">Change password</button>
              </div>
              <div className="flex justify-between items-center py-2">
            ...
            <div className="pt-4 flex justify-end">
              <button onClick={handleLogoutAllDevices} className="bg-red-500 text-white px-6 py-3 rounded-xl font-bold shadow-lg hover:bg-red-600 transition-colors">
                Log out from all devices
              </button>
            </div>
            {showPasswordModal && <ChangePasswordModal onClose={() => setShowPasswordModal(false)} onSave={handleChangePassword} saving={saving} />}
          </div>
        );
          ...
          <div className={`space-y-6 ${blurClass}`}>
            ...
            <div className="p-6 border rounded-xl bg-white flex justify-between items-center shadow-sm">
              <div>
                ...
                {editingField === "upi" ? (
                  <input className="font-medium border-b border-teal-500 outline-none"
                    value={draft.upi_id || ""} onChange={(e) => setDraft({ ...draft, upi_id: e.target.value })} placeholder="yourname@upi" />
                ...
                <div className="flex-grow">
                  ...
                  {editingField === "billingAddress" ? (
                    <textarea rows={3} className="w-full border-b border-teal-500 outline-none resize-none"
                      value={draft.billingAddress || ""} onChange={(e) => setDraft({ ...draft, billingAddress: e.target.value })} />
              ...
              {invoices.length === 0 ? (
                ...
                invoices.map((invoice) => (
                  <div key={invoice._id || invoice.id} className="p-4 border rounded-xl bg-white flex justify-between items-center">
                    ...
                    <button onClick={() => window.open(`${API_BASE}/invoices/${invoice._id}/download`, "_blank")}
