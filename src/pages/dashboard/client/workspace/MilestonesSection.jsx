...
const MilestonesSection = ({
  ...
  const [actionLoading, setActionLoading] = useState(null); // milestoneId being actioned
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap gap-6 text-xs text-gray-500">
            <span className="flex items-center gap-2">
              <Calendar size={14} />
              Started:{" "}
              <strong className="text-gray-800">{summary.startedOn}</strong>
            </span>
            <span className="flex items-center gap-2">
              <Clock size={14} />
              Deadline:{" "}
              <strong className="text-gray-800">{summary.deadline}</strong>
            </span>
            <span className="flex items-center gap-2">
              <DollarSign size={14} />
              Duration:{" "}
              <strong className="text-gray-800">{summary.duration}</strong>
            </span>
          </div>
          <button
      ...
      <div className="space-y-4">
        ...
        {milestonesList.map((ms, index) => {
          ...
          const submittedOn = ms.submittedOn
            ? new Date(ms.submittedOn).toLocaleDateString("en-IN", {
                day: "numeric",
                month: "short",
                year: "numeric",
              })
                        ...
                        <>
                          ...
                          <div className="flex gap-3">
                            <button
                              disabled={isLoading}
                              onClick={() =>
                                handleMilestoneAction(ms._id, "request_changes")
                              }
                            ...
                            <button
                              disabled={isLoading}
                              onClick={() =>
                                handleMilestoneAction(ms._id, "approve")
                              }
                      ...
                      <div className="space-y-4 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span className="font-semibold text-gray-900">
                            ₹{ms.budget.toLocaleString("en-IN")}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          ...
                          <span className="font-semibold text-gray-900">
                            ₹{(ms.budget * 0.05).toLocaleString("en-IN")}
                          </span>
                        </div>
                        <div className="border-t border-dashed pt-4 flex justify-between">
                          ...
                          <span className="font-bold text-teal-700">
                            ₹{(ms.budget * 1.05).toLocaleString("en-IN")}
                          </span>
                        </div>
                        ...
                        {ms.isPaid && (
                          <div className="bg-green-50 border border-green-100 rounded-xl p-3 text-xs text-green-700 font-semibold">
                            ✓ Paid on{" "}
                            {ms.paidAt
                              ? new Date(ms.paidAt).toLocaleDateString("en-IN")
                              : "—"}
                          </div>
                        )}
                      </div>
            ...
            <div className="relative border-l border-gray-200 ml-2 space-y-8">
              {activityTimeline.map((item, idx) => (
                <div key={idx} className="relative pl-6">
                  <span
                    className={`absolute -left-[9px] top-1 w-4 h-4 rounded-full border-4 border-white ${item.primary ? "bg-teal-600" : "bg-gray-300"}`}
                  />
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
          ...
          <div className="mt-6 bg-teal-50 border border-teal-100 rounded-2xl p-4">
            ...
            <p className="text-sm text-teal-700 leading-relaxed">
              If you face any issue with milestones or payment, our support team
              is here to help.
            </p>
