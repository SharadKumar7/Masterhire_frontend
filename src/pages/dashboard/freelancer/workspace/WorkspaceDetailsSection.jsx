// ─── FreelancerDetailsSection.jsx ────────────────────────────────────────────
// Same design as WorkspaceDetailsSection but shows CLIENT info on right side
import React from "react";
import { useNavigate } from "react-router-dom";
import { IndianRupee, Clock, User, MessageSquare, Briefcase, Star } from "lucide-react";
import { Spinner, ErrorBanner, Avatar } from "../../shared/workspace/Shared";

const FreelancerDetailsSection = ({ data, loading, error, onOpenChat }) => {
  ...
  const statusConfig = {
    ...
    published: { label: "In Progress", cls: "bg-teal-50 text-teal-700 border-teal-200" },
      ...
      <div className="w-full bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="space-y-2">
            ...
            <p className="text-sm text-gray-500">Your active project workspace</p>
          </div>
          <span className={`shrink-0 text-xs font-bold px-3 py-1 rounded-full border shadow-sm ${st.cls}`}>
            ● {st.label}
          </span>
        </div>
      </div>
        ...
        <div className="lg:col-span-8 bg-white border border-gray-100 rounded-2xl p-6 shadow-sm flex flex-col justify-between space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="border border-gray-100 rounded-2xl bg-gray-50/60 p-5 flex items-center gap-4">
              ...
              <div>
                <p className="text-[11px] uppercase tracking-wider text-gray-400 font-semibold">Project Budget</p>
            ...
            <div className="border border-gray-100 rounded-2xl bg-gray-50/60 p-5 flex items-center gap-4">
              ...
              <div>
                <p className="text-[11px] uppercase tracking-wider text-gray-400 font-semibold">Deadline</p>
                <h3 className="text-lg font-extrabold text-gray-900 mt-0.5">{deadline}</h3>
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {(data.skills || []).length > 0 ? (
                data.skills.map((skill, idx) => (
                  <span key={idx} className="bg-teal-50 text-teal-700 border border-teal-100 px-3 py-1 rounded-xl text-xs font-semibold">
                    {skill}
                  </span>
                ))
              ) : (
        ...
        <div className="lg:col-span-4">
          {client ? (
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm h-full flex flex-col justify-between">
              <div>
                ...
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="text-sm font-bold text-gray-800">Client</h3>
                </div>

                {/* Profile */}
                <div className="flex items-center gap-4 pb-5 border-b border-gray-100">
                  <div className="relative shrink-0">
                    <Avatar name={client.name} photo={client.photo} size="md" />
                    <span className="absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full bg-emerald-500 border-2 border-white" />
                  </div>
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <h4 className="font-bold text-gray-900 text-base truncate">{client.name}</h4>
                      <span className="w-4 h-4 rounded-full bg-teal-600 text-white text-[9px] font-bold flex items-center justify-center shrink-0">✓</span>
                    </div>
                    <p className="text-xs text-gray-500 mt-1">{client.title || "Client"}</p>
                    {client.rating > 0 && (
                      <div className="flex items-center gap-1 mt-2 text-xs">
                        <Star size={11} className="text-amber-400 fill-amber-400" />
                        <strong className="text-gray-900">{client.rating}</strong>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 text-center py-5 border-b border-gray-100">
                  <div>
                    <h4 className="text-sm font-bold text-gray-900">{client.totalHires ?? "—"}</h4>
                    <p className="text-[11px] text-gray-400 mt-1">Total hires</p>
                  </div>
                  <div className="border-l border-r border-gray-100">
                    <h4 className="text-sm font-bold text-gray-900">{client.activeJobs ?? "—"}</h4>
                    <p className="text-[11px] text-gray-400 mt-1">Active jobs</p>
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-gray-900">
                      {client.totalSpent ? `₹${client.totalSpent.toLocaleString("en-IN")}` : "—"}
                    </h4>
                    <p className="text-[11px] text-gray-400 mt-1">Total spent</p>
                  </div>
                </div>

                {/* Member since / location placeholder */}
                <div className="pt-5 space-y-2">
                  <p className="text-xs font-bold text-gray-700">About client</p>
                  <p className="text-xs text-gray-500 leading-relaxed">
                    {client.title || "No additional info available."}
                  </p>
                </div>
              </div>
            ...
            <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm h-full flex flex-col items-center justify-center text-center">
              ...
              <h3 className="font-bold text-gray-800 text-sm">Client info unavailable</h3>
              <p className="text-xs text-gray-400 mt-2 max-w-[220px] leading-relaxed">
                Client details will appear here once the project is active.
              </p>
            </div>
          )}
        </div>
...
export default FreelancerDetailsSection;