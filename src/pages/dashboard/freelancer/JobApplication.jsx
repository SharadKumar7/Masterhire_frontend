...
import {
  Clock, Banknote, BarChart3, Pencil, X, IndianRupee, Briefcase,
  CheckCircle2, Loader2, AlertCircle, PencilLine, Send, MessageSquare,
...
// ─── TAB 1: Job Details ───────────────────────────────────────────────────────
// ═══════════════════════════════════════════════════════════════════════════════
const JobDetailsTab = ({ job, isApplied, isSaved, onApply, onSave, bidAmount, setBidAmount }) => {
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-[2px] z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[24px] w-full max-w-sm p-6 shadow-2xl">
            ...
            <div className="flex gap-4">
              ...
              <button
                onClick={() => { onApply(null); setShowCancelModal(false); }}
      ...
      {negotiateMode && (
        <form onSubmit={handleNegotiate} className="bg-white border border-teal-200 rounded-2xl p-5 shadow-sm space-y-4">
          ...
          <div className="relative">
            ...
            <input
              type="number" value={newBid} onChange={(e) => setNewBid(e.target.value)}
...
// ─── TAB 3: Messages ──────────────────────────────────────────────────────────
// ═══════════════════════════════════════════════════════════════════════════════
const MessagesTab = ({ jobId, clientInfo }) => {
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-1 bg-[#f0f4f8]">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-gray-400">
            ...
            <p className="text-xs mt-1">Start the conversation with the client</p>
          </div>
        ) : (
          <>
...
const FreelancerJobApplication = () => {
  ...
  const handleApply = async (bidValue) => {
    try {
      ...
      // If unapplied → delete messages & go back to details tab
      if (!newApplied) {
  ...
  // ── Tabs config: Messages tab only when applied ─────────────────────────────
  const tabs = [
    { key: "details",     label: "Job Details" },
    { key: "application", label: "Application" },
    ...(isApplied ? [{ key: "messages", label: "Messages" }] : []),
  ];
  ...
  if (!job) return (
    <div className="p-20 text-center">Job not found.</div>
  );
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 py-8 bg-white min-h-screen">
      ...
      <div>
        ...
        {activeTab === "messages" && isApplied && (
          <MessagesTab jobId={jobId} clientInfo={clientInfo} />
        )}
      </div>
    </div>
  );
