...
import {
  METRICS, CHART_DATA_THIS_MONTH, CHART_DATA_LAST_MONTH,
  BREAKDOWN_DATA, TRANSACTIONS, PAYMENT_FLOW,
} from './Data.js';

import {
  MetricCard, EarningsChart, DonutChart,
  TransactionRow, FlowStep, StatusBadge, formatINR,
} from './TransactionComponents.jsx';
...
function TransactionsPanel({ transactions }) {
  ...
  const handleExport = () => {
    const csv = [
      ...
      ...filtered.map(t => [
        t.type, t.description, t.project,
        (t.isCredit ? '+' : '-') + t.amount,
        t.status, t.date, t.time,
      ]),
        ...
        {!submitted ? (
          <>
            <div className="bg-teal-50 border border-teal-200 rounded-xl p-3 text-xs text-teal-700 font-medium">
              Available balance: <span className="font-black">{formatINR(max)}</span>
            </div>
...
export default function TransactionSummary() {
  ...
  const [chartTab, setChartTab] = useState('this'); // 'this' | 'last'
  const [showWithdraw, setShowWithdraw] = useState(false);

  const chartData = chartTab === 'this' ? CHART_DATA_THIS_MONTH : CHART_DATA_LAST_MONTH;
  const availableBalance = METRICS.find(m => m.id === 'available-balance')?.value || 0;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex bg-slate-100 p-1 rounded-lg">
                <button
                  ...
                  className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${chartTab === 'this' ? 'bg-teal-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                >
                  This Month
                </button>
                <button
                  onClick={() => setChartTab('last')}
                  className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${chartTab === 'last' ? 'bg-teal-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                >
                  Last Month
                </button>
              </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-black text-slate-900">Earnings Breakdown</h2>
            </div>
            <DonutChart data={BREAKDOWN_DATA} total={124500} />
