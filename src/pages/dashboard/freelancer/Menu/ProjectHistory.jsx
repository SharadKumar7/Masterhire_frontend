              ...
              <div className="flex justify-between text-[10px] font-semibold text-slate-500">
                <span>
                  {project.status === 'In Progress'
                    ...
                    ? `${project.progress}% done at cancellation`
                    : 'Completed'}
                </span>
              </div>
                ...
                <div className="space-y-0.5">
                  {(expanded ? project.highlights : project.highlights.slice(0, 2)).map((h, i) => (
                    <div key={i} className="flex items-center gap-1.5 text-[10px] text-slate-600">
                      <CheckCircle className="w-3 h-3 text-teal-500 shrink-0" />{h}
                    </div>
                  ))}
          ...
          <div className="md:w-36 flex flex-col items-start md:items-end gap-2.5">
            <div className="text-right">
              ...
              <div className="text-[10px] text-slate-400">
                {project.status === 'Completed' ? 'Total Earnings' : 'Agreed Price'}
              </div>
            </div>
...
// ─── Main ─────────────────────────────────────────────────────────────────────
export default function FreelancerProjectHistory() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const res  = await fetch(`${BASE_URL}/api/freelancer/project-history`, {
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <div className="bg-white p-4 rounded-xl border border-teal-200 shadow-md col-span-2 md:col-span-1 flex items-center gap-4 bg-gradient-to-br from-teal-50 to-white">
            ...
            <div>
              ...
              <div className="text-[10px] text-teal-600">From {metrics.totalReviews || 0} reviews</div>
            </div>
          </div>
        </div>
